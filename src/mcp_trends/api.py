...
async def reddit_trends(payload: TrendRequest) -> SourceResult:
    return await search_reddit(payload.topic, payload.limit)
...
async def trends_by_source(
    source: Literal["hackernews", "youtube", "github", "google-linkedin", "reddit", "rss", "podcasts"],
    ...
    if source == "reddit":
        return await search_reddit(topic, limit)
    if source == "rss":
